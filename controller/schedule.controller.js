...
class ScheduleController {
    async getTeacherSchedule(res, req){
        ...
        async getTeacherScheduleByUserID(res, req){
        const id = res.params.id
        const schedule = await db.query('SELECT * FROM \"FUNC_TeacherScheduleByUserID\"($1) ORDER BY \"WeekDayNumber\", \"StartTime\"', [id])
        req.json(schedule.rows)
    }
    async getGroupScheduleByName(res, req){
        ...
        async getGroupScheduleByUserID(res, req){
        const id = res.params.id
        const schedule = await db.query('SELECT * FROM \"FUNC_GroupScheduleByUserID\"($1) ORDER BY \"WeekDayNumber\", \"StartTime\"', [id])
        req.json(schedule.rows)
    }
}
